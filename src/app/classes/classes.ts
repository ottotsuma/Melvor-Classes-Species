import { ClassesActionEvent } from './event';
import { UserInterface } from './user-interface';
import { MasteryComponent } from './mastery/mastery';
import { ClassesManager } from './manager';
import { MasteredShout, Teacher, ClassesSkillData } from './classes.types';
import { Decoder } from './decoder/decoder';
import { MasteredShouts } from './mastered-shouts';
import { ChangeType, ClassesSettings } from './settings';
...
export class Classes extends SkillWithMastery<Teacher, ClassesSkillData> {
    ...
    public readonly renderQueue = new ClassesRenderQueue();

    public activeTeacher: Teacher;
    public shouts = new MasteredShouts(this);
    public userInterface: UserInterface;
    public settings: ClassesSettings;
    ...
    public readonly manager = new ClassesManager(this, this.game);

    constructor(namespace: DataNamespace, public readonly game: Game) {
        super(namespace, 'Classes', game);
    }
    public getErrorLog () {return 'a'}
    public registerData(namespace: DataNamespace, data: ClassesSkillData) {
    ...
    public get name() {
        return getLangString('Classes_Classes_Classes');
    }
    ...
    public Master(teacher: Teacher) {
        ...
        if (!canAfford) {
            ...
            for (const modifier of this.manager.getModifiers(teacher)) {
                html += `<small class="${modifier.level === 1 ? 'text-danger' : modifier.isActive ? 'text-success' : 'classes-text-grey'}">`;
            ...
            ${getLangString('Classes_Classes_Would_You_Like_To_Equip_This_Shout')}
            <img class="teacher-icon align-middle" src="${teacher.media}" />
            ${teacher.name}
            </h5>
            ...
            html += `<h5 class="font-w600 text-danger font-size-sm mt-3 mb-1">${getLangString(
                'Classes_Classes_This_Will_Replace_The_Mastered_Shout'
            )}</h5>`;
            ...
            const getText = (shout: MasteredShout) =>
                shout
                    ? templateLangString('Classes_Classes_Replace', { name: shout.teacher.name })
                    : getLangString('Classes_Classes_Equip');
    ...
    public unlockMastery(teacher: Teacher) {
        SwalLocale.fire({
            html: '<div id="classes-mastery-container"></div>',
            showConfirmButton: false,
            showCancelButton: false,
            showDenyButton: false,
            didOpen: popup => {
                ui.create(
                    ...
                    popup.querySelector('#classes-mastery-container')
                );
            }
        });
    }
    ...
    public initSettings(settings: ClassesSettings) {
    ...
    public get actionRewards() {
        ...
        const actionEvent = new ClassesActionEvent(this, this.activeTeacher);
...
export class ClassesRenderQueue extends GatheringSkillRenderQueue<Teacher> {
    grants = false;
    gpRange = false;
