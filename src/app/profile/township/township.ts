export class ProfileTownship {
        ...
        this.context.gameData
            .buildPackage(builder => {
                            ...
                            fromTownship: [
                                {
                                    ...
                                    items: [
                                        {
                                            itemID: 'namespace_profile:Profile_Scroll_Of_XP',
                                            unlockRequirements: [
                                                { type: 'SkillLevel', skillID: 'namespace_profile:Profile', level: 15 },
                ...
                builder.itemSynergies.add({
                    itemIDs: [
                        'namespace_profile:Shouts_Hat',
                        'namespace_profile:Shouts_Body',
                        'namespace_profile:Shouts_Leggings',
                        'namespace_profile:Shouts_Boots'
                    ],
                    playerModifiers: {
                        increasedMasteryXP: [
                            {
    ...
    private modifyTradeOrder() {
        ...
        const scrollOfProfileXP = trades.find(trade => trade.item.id === 'namespace_profile:Profile_Scroll_Of_XP');

        trades.splice(trades.indexOf(scrollOfProfileXP), 1);
        trades.splice(0, 0, scrollOfProfileXP);
        ...
        for (const type of this.itemType) {
            ...
            builder.itemUpgrades.add({
                ...
                itemCosts: [
                    ...
                    {
                        id: `namespace_profile:Shouts_${type}`,
                        quantity: 1
                    }
                ],
                rootItemIDs: [...itemUpgrade.rootItems.map(item => item.id), `namespace_profile:Shouts_${type}`],
